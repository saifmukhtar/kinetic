...
fn test_pure_rust_verifies_chiavdf_proofs_multi() {
    ...
    for challenge in &challenges {
        for &iters in &iters_list {
            let proof = chia_prover
                .evaluate(challenge, iters)
                .unwrap_or_else(|e| panic!("chiavdf prover evaluation failed for iters {}: {:?}", iters, e));

            assert_eq!(proof.proof_bytes.len(), 200, "proof length must be 200 bytes");
            ...
            assert!(is_valid_pure, "Pure Rust verifier failed to verify chiavdf proof for iters {}", iters);

            // 2. Cross-verify with Chia VDF verifier
            let is_valid_chia = chia_prover
                .verify(challenge, &proof, iters)
                .unwrap_or_else(|e| panic!("chia_prover verify failed for iters {}: {:?}", iters, e));
            ...
            if let Ok(valid) = invalid_iters_res {
                assert!(!valid, "Proof should not be valid for incorrect iteration count");
            }
        }
    }
}
