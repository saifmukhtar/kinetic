...
## 1. The Hook (Taxonomy)
This crate (`kinetic-storage`) belongs to **Layer 6: Core Infrastructure & Adapters**. It is a concrete infrastructural implementor of abstract rules defined in Layer 5.
...
Isolating the heavy `redb` database engine (and WASM OPFS shims) in Layer 6 prevents the core logic from being contaminated by I/O dependencies. If the Kinetic network ever migrates to `rocksdb`, `lmdb`, or a cloud-native SQL backend, only this single Layer 6 crate needs to be rewritten or swapped out. The consensus math, core algorithms, and higher-level binaries remain completely untouched.
