...
fn patch_cargo_bin_names(network_id: &str) -> Result<()> {
    let crates = vec![
        ("kinetic-daemon", format!("{}-daemon", network_id), "src/main.rs"),
        ("kinetic-node", format!("{}-node", network_id), "src/main.rs"),
        ("kinetic-host", format!("{}-host", network_id), "src/main.rs"),
        ("kinetic-pac", format!("{}-pac", network_id), "src/main.rs"),
        ("kinetic-nrs", format!("{}-nrs", network_id), "src/bin/kinetic-nrs.rs"),
        ("kinetic-cli", network_id.to_string(), "src/main.rs"),
    ];

    for (crate_dir, new_bin_name, bin_path) in crates {
        ...
        if path.exists() {
                ...
                .and_then(|arr| arr.iter_mut().next())
            {
                ...
                bin["path"] = toml_edit::value(bin_path);
            } else {
                let mut table = toml_edit::Table::new();
                table["name"] = toml_edit::value(new_bin_name.as_str());
                table["path"] = toml_edit::value(bin_path);
                
                let mut arr = toml_edit::ArrayOfTables::new();
                arr.push(table);
                doc["bin"] = toml_edit::Item::ArrayOfTables(arr);
            }
