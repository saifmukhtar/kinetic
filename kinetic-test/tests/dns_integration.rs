...
mod tests {
    ...
    use kinetic_core::types::{Reveal, DnsZone, DnsRecord, VdfProof};
    ...
    async fn test_dns_caching_and_coalescing() {
            ...
            while let Some(cmd) = rx.recv().await {
                if let Command::ResolveRedundant { name, responder } = cmd {
                    if name == "testdns.kin" {
                        ...
                        records.insert("@".to_string(), vec![DnsRecord::A("192.168.1.100".to_string())]);
                        records.insert("www".to_string(), vec![DnsRecord::A("192.168.1.101".to_string())]);
        ...
        let response = client.query(name.clone(), hickory_proto::rr::DNSClass::IN, hickory_proto::rr::RecordType::A).await.unwrap();
        ...
        let response_www = client.query(www_name, hickory_proto::rr::DNSClass::IN, hickory_proto::rr::RecordType::A).await.unwrap();
        let elapsed = start.elapsed();
        
        assert_eq!(response_www.response_code(), hickory_proto::op::ResponseCode::NoError);
        assert!(elapsed.as_millis() < 50, "Cache lookup should be near-instant");
        
        // 3. Query NXDOMAIN for negative caching
        let bad_name = hickory_proto::rr::Name::from_str("doesntexist.kin.").unwrap();
        let response_bad = client.query(bad_name.clone(), hickory_proto::rr::DNSClass::IN, hickory_proto::rr::RecordType::A).await.unwrap();
        assert_eq!(response_bad.response_code(), hickory_proto::op::ResponseCode::NXDomain);
        
        // Query again to ensure NXDomain is cached
        let start_bad = std::time::Instant::now();
        let response_bad2 = client.query(bad_name, hickory_proto::rr::DNSClass::IN, hickory_proto::rr::RecordType::A).await.unwrap();
        let elapsed_bad = start_bad.elapsed();
        
        assert_eq!(response_bad2.response_code(), hickory_proto::op::ResponseCode::NXDomain);
        assert!(elapsed_bad.as_millis() < 50, "Negative Cache lookup should be near-instant");
    }
}
