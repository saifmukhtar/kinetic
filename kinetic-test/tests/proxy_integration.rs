...
async fn test_proxy_integration_flow() {
    ...
    let (_client_a, _handle_a) = setup_node_with_proxy(10050, key_a, vec![], true).await;

    let key_b = Keypair::generate_ed25519();
    let boot_addr = format!("/ip4/127.0.0.1/tcp/10050/p2p/{}", peer_a);
    let (client_b, _handle_b) = setup_node_with_proxy(10051, key_b, vec![boot_addr], false).await;
