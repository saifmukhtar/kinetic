...
4.  **Bicameral Rationality:** We assume that 69% of the globally distributed Governance Council will not intentionally collude to destroy the network. If a supermajority does go rogue, the ultimate fallback relies on the social assumption that the user base will coordinate a hard-fork within the 48-hour OTA timelock window.
