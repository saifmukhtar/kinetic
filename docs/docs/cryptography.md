...
Imagine a naive decentralized registry. Alice wishes to register `example.kin`. She signs a transaction saying "Register example.kin" and broadcasts it to the P2P network. 
...
This hash $C$ looks like complete gibberish. It leaks zero information about the name "example".

She takes this commitment $C$ and feeds it directly into the VDF engine as the base element $x$.
She then computes the massive repeated squaring VDF for $T$ iterations (where $T$ is derived from the length of `example.kin.`).

Because $B_{t_1}$ was generated by the Drand network only seconds before she started, Eve knows for an absolute fact that Alice could not have pre-computed this VDF. The computation must occur strictly *after* $t_1$.

### Phase 2: The Blind Commitment

After the VDF finishes (perhaps taking hours or days), Alice finally broadcasts the blind hash $C$ to the network. She waits exactly 32 seconds. This locks her position on the network timeline.

### Phase 3: The Reveal

After exactly 32 seconds have passed, Alice broadcasts the complete `Reveal` tuple to the network:

$$ \mathcal{P} = \{\text{"example.kin."}, s, B_{t_1}, T, \pi_{\text{VDF}}, \text{PubKey}_{\text{Alice}}, \text{Signature}\} $$

When Eve sees this, she finally knows Alice wants `example.kin`. But it is too late. 

If Eve wants to steal it, she must compute the massive VDF for $T$ iterations. By the time Eve finishes, Alice's claim has already been permanently embedded in the DHT.
...
When Alice includes $B_{t_1}$ in her Reveal, she proves to the entire Kinetic network that her VDF computation began *after* the 30-second window when $B_{t_1}$ was released. By broadcasting the commitment hash exactly 32 seconds before the Reveal, she locks the front-running barrier. 
