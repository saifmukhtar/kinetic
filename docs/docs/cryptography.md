...
After the VDF finishes (perhaps taking hours or days), Alice finally broadcasts the blind hash $C$ to the network. She waits exactly 9 seconds (3 Drand pulses). This locks her position on the network timeline.
...
When Alice includes $B_{t_1}$ in her Reveal, she proves to the entire Kinetic network that her VDF computation began *after* the 3-second window when $B_{t_1}$ was released. By broadcasting the commitment hash exactly 9 seconds before the Reveal, she locks the front-running barrier. 
