...
pub enum ActionCommands {
    /// Broadcast a pre-signed action JSON file generated by kinetic-OS
    Broadcast {
...
/// Reads a pre-signed action message from a JSON file and broadcasts it to the network via the local daemon.
///
/// # Errors
...
pub async fn handle_action_command(
    ...
    pb.set_message("Publishing action action...");
    ...
    if publish_resp.status().is_success() {
        println!("Successfully published action action to the Kinetic Network!");
    } else {
        let err_text = publish_resp.text().await?;
        println!("Failed to publish action action: {}", err_text);
    }
