...
pub async fn handle_peer_id(config: &KineticConfig, client: &reqwest::Client) -> anyhow::Result<()> {
    let port = config.daemon.api_port;
    let url = format!("http://{}:{}/api/v1/micro/network/peer-id", config.daemon.bind_ip, port);
