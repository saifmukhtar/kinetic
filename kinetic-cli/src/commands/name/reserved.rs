...
pub async fn handle_reserved(config: &KineticConfig, client: &reqwest::Client) -> anyhow::Result<()> {
    let port = config.daemon.api_port;
    let url = format!("http://{}:{}/api/v1/micro/nrs/reserved", config.daemon.bind_ip, port);
    ...
    if !resp.status().is_success() {
        anyhow::bail!("Failed to fetch reserved names: {}", resp.text().await?);
    }
