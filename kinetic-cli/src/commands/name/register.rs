...
pub async fn handle(name: String, iterations: u64, config: &KineticConfig, client: &Client) -> anyhow::Result<()> {
            ...
            let final_kid_str = if kid_str.is_empty() {
                ...
                let did_string = format!("{}{}", kinetic_core::constants::DID_PREFIX, hex::encode(hasher.finalize()));
                let did = kinetic_kid::did::KineticDid::new(&did_string).map_err(|e| anyhow::anyhow!("Invalid DID: {}", e))?;
                ...
                let signed_doc = doc.sign(&kid_keypair).map_err(|e| anyhow::anyhow!("Failed to sign KID doc: {}", e))?;
                ...
                let key_path = kid_dir.join(format!("{}.key{}", base_name, kinetic_core::constants::TLD_SUFFIX));
