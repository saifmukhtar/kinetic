...
pub async fn handle_name_register(
    ...
    let label = label.strip_suffix(kinetic_core::constants::NSP_SUFFIX).unwrap_or(&label);
    ...
    if !label.is_empty() && label.len() <= 6 {
        ...
        println!("This name requires {} iterations and will take approximately {}{}.", actual_iterations, time_str, rating_str);
    }
    ...
    loop {
        ...
        if status == "Complete" {
            ...
            println!("✅ Success! {} has been completely registered and published.", fqdn);
