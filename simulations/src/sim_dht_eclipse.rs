...
fn main() {
    ...
    println!(
        "Simulating {} billion names ({} billion total derived keys).",
        TOTAL_NAMES as f64 / 1e9,
        (TOTAL_NAMES * KEYS_PER_NAME) as f64 / 1e9
    );
    println!(
        "Measuring distribution across {} Kademlia sectors (top 16 bits).",
        NUM_BUCKETS
    );
    ...
    println!(
        "Variance (Max - Min): {:.2}%",
        ((max_count - min_count) as f64 / expected_avg) * 100.0
    );
