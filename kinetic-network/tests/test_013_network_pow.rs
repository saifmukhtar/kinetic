...
fn test_pow_difficulty_1() {
    ...
    let keypair = mine_sybil_keypair(Kyn(kyn), difficulty);
    let peer_id = PeerId::from(keypair.public());
    assert!(is_valid_sybil_pow(&peer_id, Kyn(kyn), difficulty));
}
...
fn test_pow_invalid_nonce() {
    ...
    while is_valid_sybil_pow(&peer_id, Kyn(wrong_kyn), difficulty) {
        wrong_kyn += 100000;
    }
    assert!(!is_valid_sybil_pow(&peer_id, Kyn(wrong_kyn), difficulty));
}
...
fn test_pow_invalid_payload() {
    ...
    let tampered_peer = loop {
        ...
        if !is_valid_sybil_pow(&p, Kyn(kyn), difficulty) {
            break p;
        }
    };

    assert!(!is_valid_sybil_pow(&tampered_peer, Kyn(kyn), difficulty));
}
