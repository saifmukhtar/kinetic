...
async fn test_03_test_mode_flag_applied() {
    ...
    let keypair = Keypair::generate_ed25519();
    let (storage, vdf_engine) = create_engine_and_store();
    let (_, event_loop) = NetworkEventLoop::new(
        config,
        keypair,
        storage,
        watch::channel(0).1,
...
async fn test_04_bad_bootstrap_peer_ignored() {
    ...
    assert!(
        result.is_ok(),
        "Swarm builder should not panic on unreachable bootstrap peers"
    );
}
...
async fn test_06_banned_peers_loaded_properly() {
    ...
    let mut key = kinetic_core::constants::DB_PREFIX_BANNED_PEER
        .as_bytes()
        .to_vec();
    ...
    let (_, mut event_loop) = NetworkEventLoop::new(
        config,
        keypair,
        storage,
        watch::channel(0).1,
...
async fn test_07_expired_bans_cleared() {
    ...
    assert!(
        !event_loop.is_banned(&peer_id),
        "Expired bans should be ignored"
    );
}
...
async fn test_09_quic_fallback() {
    ...
    assert!(
        result.is_ok(),
        "QUIC failure should fallback gracefully to TCP"
    );
}
...
async fn test_12_gossip_ban_is_per_peer() {
    ...
    assert!(
        !event_loop.is_banned(&peer_b),
        "peer_b should not be banned yet"
    );
}
...
async fn test_13_gossip_semaphore_not_zero_bound() {
    ...
    assert!(
        permit.is_ok(),
        "gossip_semaphore must have available permits on startup"
    );
}
