...
pub fn is_valid_sybil_pow(peer_id: &PeerId, current_kyn: kinetic_types::clock::Kyn, difficulty: u32) -> bool {
...
mod tests {
    ...
    fn test_pow_mining_and_validation() {
        ...
        assert!(is_valid_sybil_pow(&peer_id, kinetic_types::clock::Kyn(kyn), difficulty));

        // Should be valid for kyn at the very end of the current epoch
        let end_of_epoch_kyn = (kyn / EPOCH_KYNS) * EPOCH_KYNS + EPOCH_KYNS - 1;
        assert!(is_valid_sybil_pow(&peer_id, kinetic_types::clock::Kyn(end_of_epoch_kyn), difficulty));
        ...
        assert!(is_valid_sybil_pow(&peer_id, kinetic_types::clock::Kyn(next_epoch_kyn), difficulty));

        // Should NOT be valid for kyn 2 epochs away (unless we get a 1/256 lucky collision)
        let two_epochs_away = kyn + (2 * EPOCH_KYNS);
        if is_valid_sybil_pow(&peer_id, kinetic_types::clock::Kyn(two_epochs_away), difficulty) {
            println!("Random collision for two_epochs_away - skipping assert");
        } else {
            assert!(!is_valid_sybil_pow(&peer_id, kinetic_types::clock::Kyn(two_epochs_away), difficulty));
        }

        // Should NOT be valid for kyn 1 epoch ago
        if kyn > EPOCH_KYNS {
            let prev_epoch_kyn = kyn - EPOCH_KYNS;
            if is_valid_sybil_pow(&peer_id, kinetic_types::clock::Kyn(prev_epoch_kyn), difficulty) {
                println!("Random collision for prev_epoch_kyn - skipping assert");
            } else {
                assert!(!is_valid_sybil_pow(&peer_id, kinetic_types::clock::Kyn(prev_epoch_kyn), difficulty));
            }
        }
    }
}
