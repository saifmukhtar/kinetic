    ...
    match event {
        Event::Message { peer, message, .. } => match message {
            Message::Request { channel, .. } => {
                ...
                let response = ActionSyncResponse {
                    actions: swarm.action_log.clone(),
                };
                let _ = swarm
                    .swarm
                    .behaviour_mut()
        ...
        Event::OutboundFailure {
            ...
            debug!(
                "ActionSync outbound failure to peer {:?}: {:?}",
                peer, error
            );
            if let Some(responder) = swarm.pending_action_sync_requests.remove(&request_id) {
                let _ = responder.send(Err(crate::client::types::ProxyError::Other(
                    std::borrow::Cow::Owned(error.to_string()),
                )));
            }
        }
        Event::InboundFailure { peer, error, .. } => {
            debug!(
                "ActionSync inbound failure from peer {:?}: {:?}",
                peer, error
            );
        }
