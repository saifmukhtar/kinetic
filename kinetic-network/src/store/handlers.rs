...
impl KineticRecordStore {
    pub(crate) fn handle_put_record(
        ...
        if let Some(reveal) = reveal_ref {
            ...
            if effective_age > kinetic_core::types::RESQUARING_EPOCH_KYNS {
                ...
                err.log_warning("KIN-PRV-001", record.name(), "Rejecting Record:");
                return Err(err);
            }
        }
        ...
        {
            e.log_warning("KIN-PRV-002", record.name(), "Rejecting Reveal:");
            return Err(e);
        }
            ...
            if existing_record.pubkey() != record.pubkey() {
                ...
                let (existing_reveal, new_reveal) = match (existing_record, record) {
                    ...
                    _ => {
                        ...
                        err.log_warning("KIN-PRV-009", record.name(), "Rejecting Steal:");
                        return Err(err);
                    }
                };
                ...
                if new_reveal.iterations == existing_reveal.iterations && hb_age < 100 {
                    ...
                    err.log_warning("KIN-PRV-004", &new_reveal.name, "Rejecting Steal Reveal:");
                    return Err(err);
                } else {
                    tracing::info!(
                ...
                if new_pulse < existing_pulse {
                    ...
                    err.log_warning("KIN-PRV-010", record.name(), "Rejecting Replayed Reveal:");
                    return Err(err);
        ...
        if deque.len() >= self.max_reveals_per_hour {
            ...
            err.log_warning("KIN-PRV-011", name, "Rejecting Reveal:");
            return Err(err);
        }
        deque.push_back(now);
    ...
    pub(crate) fn handle_process_heartbeat(
        ...
        if heartbeat.latest_kyn < existing_pulse {
            ...
            err.log_warning("KIN-PRV-007", &heartbeat.name, "Rejecting Heartbeat:");
            return Err(err);
        }
        ...
        let existing_record = match self.get_fallback(&heartbeat.name) {
            ...
            None => {
                ...
                err.log_warning("KIN-PRV-005", &heartbeat.name, "Rejecting Heartbeat:");
                return Err(err);
            }
        };
        ...
        let is_valid_signature = if let Some(auth) = &heartbeat.authorization {
            if kinetic_primitives::verify_mldsa(
                ...
                err.log_warning("KIN-PRV-012", &heartbeat.name, "Rejecting Heartbeat:");
                return Err(err);
            }
            ...
            if !has_cap {
                ...
                err.log_warning("KIN-PRV-008", &heartbeat.name, "Rejecting Heartbeat:");
                return Err(err);
            }

            let kid_doc = auth.kid_doc.as_ref().ok_or_else(|| {
                let err = KineticStoreError::MissingKidDocument;
                err.log_warning("KIN-PRV-013", &heartbeat.name, "Rejecting Heartbeat:");
                err
            })?;
        ...
        if !is_valid_signature {
            ...
            err.log_warning("KIN-PRV-006", &heartbeat.name, "Rejecting Heartbeat:");
            return Err(err);
        }
