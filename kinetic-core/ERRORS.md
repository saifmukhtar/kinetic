...
| [KIN-NAM](#kin-nam--domain-name-validation) | Domain Name Validation | `NamesError` |
...
**Solution:** Use only lowercase letters (`a–z`), digits (`0–9`), and internal hyphens. The name must end with `.kin` and cannot be a subdomain (e.g. `blog.example.kin` is not registerable directly). See [KIN-NAM errors](#kin-nam--domain-name-validation) for specifics.
...
**Solution:** Ensure the CNAME target is a valid fully-qualified domain name of 1–253 characters.
...
## KIN-NAM — Domain Name Validation

Errors from domain name structural validation. These are returned before any network operation when a submitted name violates Kinetic naming rules.
...
**What it is:** The full normalized domain name (including `.kin` suffix) exceeds 253 characters or is empty.

**Why it occurs:** RFC 1035 §2.3.4 defines a maximum of 253 characters for a fully-qualified domain name.
...
**What it is:** A single dot-separated label within the domain exceeds 63 characters or is empty.
...
**What it is:** The submitted name is a subdomain (e.g. `blog.example.kin`) rather than an apex domain (`example.kin`).
...
**Solution:** Register the apex domain (`example.kin`) and then configure the subdomain (`blog.example.kin`) inside that domain's DNS zone payload.
