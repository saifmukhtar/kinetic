...
fn test_002_silent_identity_wipe() {
    ...
    let id_file = temp_dir.join("identity.key");

    // 2. Create a corrupted identity.key (e.g. 10 bytes instead of 32)
    ...
    // Under the OLD logic (flawed), this would silently overwrite identity.key with a new 32-byte key
    // and return Ok().
