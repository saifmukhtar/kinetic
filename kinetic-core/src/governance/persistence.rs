        ...
        match std::fs::File::open(path) {
            Ok(file) => match bincode::deserialize_from(file) {
                ...
                Err(e) => {
                    ...
                    tracing::error!(
                        "KIN-ACN-036: CRITICAL: Governance state corrupted: {}. Refusing to start with a reset state.",
                        e
                    );
                    panic!(
            ...
            Err(e) => {
                tracing::error!("KIN-ACN-037: CRITICAL: Failed to read Governance state file: {}.", e);
                panic!(
