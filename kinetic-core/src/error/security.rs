...
impl SecurityError {
    ...
    pub fn user_message(&self) -> String {
        match self {
            ...
            Self::NrsSsrfBlocked => {
                "Security block: Apex name resolved to a prohibited internal IP address."
                    .to_string()
            }
            Self::PathTraversalAttempt => {
                "Security block: Malicious path traversal attempt blocked.".to_string()
            }
            ...
            Self::BackendResponseTooLarge => {
                "Security block: Backend response payload size exceeded.".to_string()
            }
            Self::DangerousIpBlocked => {
                "Security block: Target resolved to a dangerous IP address.".to_string()
            }
            Self::DevModePrivateIp => {
                "Warning: Forwarding to a private IP (allowed in dev mode only).".to_string()
            }
            Self::ProxyLoop => {
                "Security block: Detected a loop attempting to connect to the node's own port."
                    .to_string()
            }
        }
    }
