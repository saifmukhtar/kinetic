...
## 1. The Hook (Taxonomy)
This crate (`kinetic-vdf`) belongs to **Layer 4: Infrastructure**. It is a concrete infrastructural implementor of abstract rules defined in Layer 3.
...
Isolating the heavy RSA cryptography in Layer 4 prevents the core kernel and other infrastructural components from being weighed down by mathematical dependencies. If Kinetic decides to swap out RSA VDFs for Class Group VDFs or ASIC-based delays in the future, only this single Layer 4 crate needs to be rewritten, leaving the rest of the workspace completely untouched.
