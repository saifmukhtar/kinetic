...
mod tests {
    ...
    fn test_chiavdf_and_kyn_vdf_differential_compatibility() {
        ...
        for (index, hash) in challenges.into_iter().enumerate() {
            ...
            let kyn_vdf_corrupted_rejected =
                match kyn_vdf::verify_chia_vdf(&challenge.hash, &corrupted_bytes, iterations, 1024)
                {
                    Ok(false) => true,
                    Ok(true) => false,
                    Err(_) => true,
            ...
            let kyn_vdf_wrong_iterations_rejected = match kyn_vdf::verify_chia_vdf(
                &challenge.hash,
                &proof.proof_bytes,
                wrong_iterations,
                1024,
            ...
            let wrong_challenge = Commitment { hash: wrong_hash };
            ...
            let kyn_vdf_wrong_challenge_rejected = match kyn_vdf::verify_chia_vdf(
            ...
            assert!(!chiavdf_truncated, "chiavdf accepted a truncated proof");

            let kyn_vdf_truncated_rejected =
                match kyn_vdf::verify_chia_vdf(&challenge.hash, &truncated_bytes, iterations, 1024)
                {
                    Ok(false) => true,
                    Ok(true) => false,
                    Err(_) => true,
