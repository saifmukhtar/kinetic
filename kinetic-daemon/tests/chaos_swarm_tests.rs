...
fn test_chaos_swarm_nodes() {
    let mut sim = Builder::new().simulation_duration(std::time::Duration::from_secs(15)).build();
    let num_daemons = 100;
    ...
    sim.client("client", async move {
        ...
        assert_eq!(successful, num_daemons, "All {} daemons should be online", num_daemons);
        println!("✅ PHASE 1: Successfully pinged all {} daemons.", num_daemons);
        ...
        println!("✅ PHASE 3: Chaos resolved! {} survived, {} safely failed.", alive_count, dead_count);
        assert_eq!(alive_count, num_daemons / 2, "Exactly half the nodes should survive");
        assert_eq!(dead_count, num_daemons / 2, "Exactly half the nodes should fail gracefully");
