...
impl ProxyConfigurator for FallbackConfigurator {
    ...
    fn save_previous_state(&self) -> Result<SavedState, ProxyConfigError> {
        Ok(SavedState { previous_pac_url: None, proxy_type: None })
    }
...
impl ProxyConfigurator for KdeConfigurator {
    fn install(&self, pac_url: &str) -> Result<(), ProxyConfigError> {
        ...
        Command::new("kwriteconfig5")
            .args(["--file", "kioslaverc", "--group", "Proxy Settings", "--key", "Proxy Config Script", pac_url])
            .status()
            .map_err(|e| ProxyConfigError::Command(format!("kwriteconfig5 failed: {}", e)))?;
    ...
    fn uninstall(&self) -> Result<(), ProxyConfigError> {
        ...
        Command::new("kwriteconfig5")
            ...
            .status()
            .map_err(|e| ProxyConfigError::Command(format!("kwriteconfig5 uninstall failed: {}", e)))?;
    ...
    fn restore_state(&self, state: &SavedState) -> Result<(), ProxyConfigError> {
        if let Some(ref proxy_type) = state.proxy_type {
            Command::new("kwriteconfig5")
                .args(["--file", "kioslaverc", "--group", "Proxy Settings", "--key", "ProxyType", proxy_type])
                .status()
                .map_err(|e| ProxyConfigError::Command(format!("kwriteconfig5 restore failed: {}", e)))?;
        }
...
impl ProxyConfigurator for GnomeConfigurator {
    fn install(&self, pac_url: &str) -> Result<(), ProxyConfigError> {
        ...
        Command::new("gsettings")
            .args(["set", "org.gnome.system.proxy", "autoconfig-url", &format!("'{}'", pac_url)])
            .status()
            .map_err(|e| ProxyConfigError::Command(format!("gsettings failed: {}", e)))?;
    ...
    fn save_previous_state(&self) -> Result<SavedState, ProxyConfigError> {
        Ok(SavedState { previous_pac_url: None, proxy_type: Some("'none'".to_string()) })
    }
...
impl PacManager {
    ...
    pub fn install(&self, pac_url: &str) -> Result<(), ProxyConfigError> {
        ...
        if self.lock_path.exists() {
            match File::open(&self.lock_path).and_then(|f| Ok(serde_json::from_reader::<_, SavedState>(f))) {
                Ok(Ok(saved)) => {
...
"#.trim().to_string();
    ...
    let app = Router::new().route("/proxy.pac", get(move || async move {
